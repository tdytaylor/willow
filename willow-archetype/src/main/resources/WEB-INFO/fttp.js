...
if ($treenum == 0) {
  ...
  for (var i = 0; i < $fttpchild.length; i++) {
    ...
    result += $fttpchild[i].isFile() ? fem(name, pathcode, size, tm) : dem(
        geteid(i), name, pathcode, readable, fcs, tm);
    /*
...
function fem(n, pc, size, tm) {
  return getblank($treenum + 1) + "<font color=#1F6CBE>" + n + "</font>" + (size
      ? blankStr + size + "byte" : "") + (tm ? blankStr + tm : "")
      + "<input type=radio name='treeId' value='tid=" + pc + "&file=true'><br>";
}

function dem(eid, n, pc, rd, fcs, tm) {
  return getblank($treenum + 1) + "<a id=p" + eid + " href='javascript:" + (fcs
      ? "menuClick(p" + eid + ")" : "void(0)") + "' tid='" + pc + "' tn='"
      + ($treenum + 1) + "' " + (fcs ? "" : "disabled") + "><b>" + n
      + "</b></a> " + (tm ? blankStr + tm : "")
      + "<input type=radio name='treeId' value='treeId=" + pc + "' " + (fcs ? ""
          : "disabled") + "><br><div id=c" + eid
      + " style='display:none' loaded='no'>" + getblank($treenum + 1) + blankStr
      + loadingStr + "</div>";
}
...
function geteid(bstr) {
  var d = new Date();
  return bstr + d.getHours() + "" + d.getMinutes() + "" + d.getSeconds() + ""
      + d.getMilliseconds();
}
