...
import com.fourinone.ConstantBit.Target;
...
class DumpWorker extends MigrantWorker implements CoolHashBase {
  ...
  private final List<String[]> pla = new ArrayList<String[]>();
  ...
  private Object dump(String d, String[] k, Object[] v, Filter f, byte c, boolean p,
      String... psk) {
    ...
    if (da.exists() && da.length() != 0) {
      ...
      for (int i = 0; i < metagroup.length; i++) {
          ...
          while (brp.reading()) {
            ...
            if (ks > 0) {
              ...
              if (vs > 0) {
                ...
                if (kl.contains(kcs)) {
                  if (c > 0x28 && index == -1) {
                    index = brp.getReadIndex() - ks - vs - DumpAdapter.ConstBit[1]
                        - DumpAdapter.ConstBit[2];
                  }
                  if (p && vbt[vbt.length - 1] == (byte) DumpAdapter.ConstBit[8]) {
                    StringBuilder pk = (new StringBuilder(
                        Target.getTargetObject(vbt, String.class)))
                        .append(psk != null && pla.size() < psk.length ? psk[pla.size()] : "");
        ...
        dar.close(dal);
        if (cm.size() == kl.size() || (cm.containsKey("0x1e") && (int) cm.get("0x1e") == kl
            .size())) {
          break;
        }
      }
      if (cmp.size() > 0) {
        ...
        String[] vlstr = cmp.values(String.class).toArray(new String[0]);
    ...
    if (c > 0x2a) {
      ...
      while (j < kl.size()) {
        ...
        while (j < kl.size()) {
          //v = dumpHandle(vl,v) 2017.10
          int ns = k[j].length() + ((byte[]) v[j]).length + DumpAdapter.ConstBit[1]
              + DumpAdapter.ConstBit[2];
          if (ns + bwp.getBytesLength() + s + DumpAdapter.ConstBit[2] <= DumpAdapter.ms) {
            bwp.writeShort((short) k[j].length()).writeChars(k[j]).writeInt(((byte[]) v[j]).length)
                .writeBytes((byte[]) v[j]);
            r++;
          } else {
            break;
  ...
  private Object dump(String d, String w, Filter f, byte c, boolean p, String... psk) {
    ...
    if (dal.being()) {
                ...
                if (p && vbt[vbt.length - 1] == (byte) DumpAdapter.ConstBit[8]) {
                  ...
                  try {
                    if (f == null || f.match(vbt)) {
                      if (checkMatch(kcs, w)) {
                        if (c > 0x1e) {
                          ((CoolHashMap) dumpobj).putValue(kcs, vbt);
                        } else {
                          ((CoolHashMap.CoolKeySet) dumpobj).addKey(kcs);
                        }
                      }
                    }
                  } catch (Exception ex) {
      ...
      if (cmp.size() > 0) {
        ...
        String[] vlstr = cmp.values(String.class).toArray(new String[0]);
        if (psk != null) {
          psk = psk.length > 1 ? Arrays.copyOfRange(psk, 1, psk.length)
              : null;//psk=Arrays.copyOfRange(psk,1,psk.length-1);
        }
        WareHouse wh = da.getKeyMetaStr(vlstr);
        Set<Entry> meset = (Set<Entry>) wh.entrySet();
        for (Entry me : meset) {
          ...
          for (int i = 0; i < vlstr.length; i++) {
            if (cmcf.containsKey(vlstr[i])) {
              if (c > 0x1e) {
                ((CoolHashMap) dumpobj).putValue(klstr[i], cmcf.getValue(vlstr[i]));
              } else {
                ((CoolHashMap.CoolKeySet) dumpobj).addKey(klstr[i]);
              }
            }
          }
        }
      ...
      if (!fwstr.equals("\u002A")) {
        if (!fwstr.equals(knstr)) {
          ...
          if (at == -1 || fwstr.length() > knstr.length()) {
            ...
            return false;
          } else if (at > 0 && (!knstr.endsWith(fwstr.substring(at + 1)) || !knstr
              .startsWith(fwstr.substring(0, at)))) {
            return false;
          }
        }
      }
    ...
    if (c == 0x2) {
      if (!p && psk[0] != null) {
        ...
        if (vl != null && vl[vl.length - 1] == (byte) DumpAdapter.ConstBit[9]) {
          CoolBitSet cbs = Target.getTargetObject(vl);
          i = cbs.get(Integer.parseInt(psk[0])) ? 1 : 0;
        }
        vl = Target.getTargetBytes(i);
      }
    }
    return vl;
  }
  ...
  byte[][] dumpHandle(byte[] vl, byte[] v, byte c, boolean p, String... psk) {
    ...
    if (c == 0x18) {
      Object value = chex.getCheckPlus(Target.getTargetObject(vl), Target.getTargetObject(v));
      v = value != null ? Target.getTargetBytes(value) : vl;
    }
    if (c == 0x1c) {
      CoolBitSet cbs = null;
      if (vl != null && vl[vl.length - 1] == (byte) DumpAdapter.ConstBit[9]) {
        cbs = Target.getTargetObject(vl);
      } else {
        cbs = new CoolBitSet();
      }
      int i = Target.getTargetObject(v);
      vl = Target.getTargetBytes(cbs.put(i));
      v = Target.getTargetBytes(cbs);
    ...
    if (c == 0x1d) {
      if (!p && psk[0] == null) {
        CoolBitSet cbsv = Target.getTargetObject(v);
        if (vl != null && vl[vl.length - 1] == (byte) DumpAdapter.ConstBit[9]) {
          CoolBitSet cbsvl = Target.getTargetObject(vl);
          int n = cbsvl.set(cbsv);
          vl = Target.getTargetBytes(n);
          v = Target.getTargetBytes(cbsvl);
        } else {
          vl = Target.getTargetBytes(cbsv.getTotal());
        }
      }
      if (!p && psk[0] != null) {
        if (vl != null && vl[vl.length - 1] == (byte) DumpAdapter.ConstBit[9]) {
      ...
      if (c < 0x1e) {
        ...
        if (c < 0x28) {
          ...
          if (c > 0x1e) {
            ...
            wh.put(0x14, ((CoolHashMap.CoolKeySet) dumpobj).toArray(new String[0]));
          }
        } else if (c < 0x3c) {
  ...
  void waitWorking(String[] args) {
    ...
    if (args != null && args.length == 2) {
      ...
      waitWorking(args[0], Integer.parseInt(args[1]), args[2], Integer.parseInt(args[3]),
          "DataWorker");
    }
  }
}
