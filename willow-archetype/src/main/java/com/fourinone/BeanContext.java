...
public class BeanContext extends ServiceContext {
  ...
  public static ParkLocal getPark() {
    ...
    return getPark(parkcfg[0][0], Integer.parseInt(parkcfg[0][1]),
        parkcfg);//input serverconfiglist string[][]
  }
...
//      LogUtil.info("[BeanContext]", "[startPark]", e);
  ...
  static void startWorker(String host, int port, String sn, MigrantWorker mwk, boolean issup) {
    try {
      ...
      if (issup) {
        startService(host, port, sn, pa, ConfigContext.getInetStrConfig(mwk.getWorkerJar()),
            ConfigContext.getPolicyConfig());
      } else {
  ...
  static void startFttpWorker(String host, int port, String sn, MigrantWorker mwk) {
    try {
      ParkActive pa = (ParkActive) DelegateConsole.bind(new Class[]{Worker.class, FttpWorker.class},
          new FttpWorkerService(mwk));//Worker.class
  ...
  static WorkerLocal getWorkerLocal(String host, int port, String sn) {
    return (WorkerLocal) DelegateConsole.bind(new Class[]{WorkerLocal.class, CtorLocal.class},
        new WorkerServiceProxy(host, port, sn));
  }
  ...
  static WorkerLocal getWorkerLocal() {
    return (WorkerLocal) DelegateConsole
        .bind(new Class[]{WorkerLocal.class}, new WorkerLocalProxy());
  }

  static WorkerLocal getFttpLocal(String host, int port, String sn) {
    return (WorkerLocal) DelegateConsole
        .bind(new Class[]{WorkerLocal.class, CtorLocal.class, FttpLocal.class},
            new FttpWorkerProxy(host, port, sn));
  }
  ...
  public static void startCacheFacade(String host, int port) {
    try {
      startService(host, port, ConfigContext.getCacheFacadeService(), DelegateConsole
          .bind(Cache.class, new CacheFacade(ConfigContext.getCacheService(),
              ConfigContext.getCacheGroupConfig())));
    } catch (RemoteException e) {
  ...
  */
  ...
  public static void startCoolHashServer(String parkhost, int parkport, int worknum) {
    startCoolHashServer("com.fourinone.CoolHashWorker", parkhost, parkport, worknum,
        "fourinone.jar" + File.pathSeparator, "log", null);
  }

  public static void startCoolHashServer(String parkhost, int parkport, int worknum,
      String jarclasspath, String logdir, String langCode) {
    startCoolHashServer("com.fourinone.CoolHashWorker", parkhost, parkport, worknum, jarclasspath,
        logdir, langCode);
  }
  ...
  public static void startCoolHashServer(String parkhost, int parkport, int worknum,
      String langCode) {
    startCoolHashServer("com.fourinone.CoolHashWorker", parkhost, parkport, worknum,
        "fourinone.jar" + File.pathSeparator, "log", langCode);
  }

  static void startCoolHashServerLocal(String parkhost, int parkport, int worknum) {
    startCoolHashServer("com.fourinone.DumpWorker", parkhost, parkport, worknum,
        "fourinone.jar" + File.pathSeparator, "log", null);
  }

  static void startCoolHashServerLocal(String parkhost, int parkport, int worknum,
      String jarclasspath, String logdir) {
    startCoolHashServer("com.fourinone.DumpWorker", parkhost, parkport, worknum, jarclasspath,
        logdir, null);
  }

  private static void startCoolHashServer(String service, String parkhost, int parkport,
      int worknum, String jarclasspath, String logdir, String langCode) {
    try {
      ...
      starts[0] = BeanContext
          .tryStart("java", "-cp", jarclasspath, "com.fourinone.BeanContext", parkhost,
              parkport + "");
      ...
      for (int i = 1; i < starts.length; i++) {
        if (langCode != null) {
          starts[i] = BeanContext
              .tryStart("java", "-Dfile.encoding=" + langCode, "-cp", jarclasspath, service,
                  parkhost, parkport + "", parkhost, (port++) + "");
        } else {
          starts[i] = BeanContext
              .tryStart("java", "-cp", jarclasspath, service, parkhost, parkport + "", parkhost,
                  (port++) + "");
        }
