...
public class FileAdapter extends File {
  ...
  public static final short[] ConstBit = new short[]{0 << 1, 1 << 1, 1 << 2, 1 << 3, 3 << 1, 5 << 1,
      6 << 1, 7 << 1, 1 << 0, 8 << 1};//2017.10
  private final List<FileAdapter> closeList = new ArrayList<FileAdapter>();
  FileLock flk = null;
  ...
  void close(FileAdapter fa) {
    if (!equals(fa)) {
      ...
      fa.closeList.add(this);
    }
  }

  public interface ByteReadParser {

    byte[] read(int totalnum);

    byte[] readLine();

    byte[] read(byte[] split);

    byte[] readLast(byte[] split);
    ...
    byte[] readCharsBytes(int length);

    String readChars(int length);

    Object readObject(int length);

    ByteReadParser reset(byte[] array);
  ...
  public interface ByteReadAdapter extends ByteReadParser, TryByteReadAdapter {
    ...
    byte[] readAllSafety() throws FileException;
  }
  ...
  public interface IntReadAdapter extends TryIntReadAdapter {
    ...
    int[] readIntAllSafety() throws FileException;

    int[] readInt(int totalnum);

    int readInt();

    List<Integer> readListIntAll();

    List<Integer> readListInt(int totalnum);
  }
  ...
  public interface IntWriteAdapter extends TryIntWriteAdapter {
    ...
    int writeInt(int[] its);

    int writeIntSafety(int[] its) throws FileException;

    int writeListInt(List<Integer> ls);
  }
  ...
  public interface ByteWriteAdapter extends TryByteWriteAdapter {
    ...
    int writeSafety(byte[] bytes) throws FileException;
  }
  ...
  public interface ByteWriteParser {
    ...
    ByteWriteParser writeBytes(byte[] bts);

    ByteWriteParser writeShort(short n);

    ByteWriteParser writeInt(int n);

    ByteWriteParser writeDouble(double n);

    ByteWriteParser writeLong(long n);

    ByteWriteParser writeFloat(float n);

    ByteWriteParser writeDate(Date time);

    ByteWriteParser writeString(String n);

    ByteWriteParser writeCharsBytes(byte[] bts);

    ByteWriteParser writeChars(String n);

    ByteWriteParser writeObject(Object n);

    int getBytesLength();

    byte[] getBytes();

    ByteWriteParser reset();

    ByteWriteParser reset(int size);
  }
