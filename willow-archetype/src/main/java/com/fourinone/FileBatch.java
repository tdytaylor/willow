...
public class FileBatch {
  ...
  private Result[] readAllBatch(TryByteReadAdapter[] tbrs, TryIntReadAdapter[] tirs,
      boolean locked) {
    Result[] hmarr = new Result[tbrs != null ? tbrs.length : tirs.length];
    for (int i = 0, j = 0; j < hmarr.length; ) {
      if (hmarr[i] == null) {
        hmarr[i] = tbrs != null ? (locked ? tbrs[i].tryReadAllSafety() : tbrs[i].tryReadAll())
            : (locked ? tirs[i].tryIntReadAllSafety() : tirs[i].tryIntReadAll());
  ...
  public Result<Integer>[] writeBatch(TryByteWriteAdapter[] fwas, List<byte[]> btarr,
      boolean locked) {
    return writeBatch(fwas, btarr, null, locked);
  }

  private Result<Integer>[] writeBatch(TryByteWriteAdapter[] fwas, List<byte[]> btarr, byte[] bts,
      boolean locked) {
  ...
  public Result<Integer>[] writeIntBatch(TryIntWriteAdapter[] fwas, List<int[]> btarr,
      boolean locked) {
    return writeIntBatch(fwas, btarr, null, locked);
  }

  private Result<Integer>[] writeIntBatch(TryIntWriteAdapter[] fwas, List<int[]> btarr, int[] its,
      boolean locked) {
    return writeBatch(null, fwas, btarr, null, its, locked);
  }

  private Result<Integer>[] writeBatch(TryByteWriteAdapter[] fbws, TryIntWriteAdapter[] fiws,
      List btarr, byte[] bts, int[] its, boolean locked) {
    Result[] hmarr = new Result[fbws != null ? fbws.length : fiws.length];
    for (int i = 0, j = 0; j < hmarr.length; ) {
      if (hmarr[i] == null) {
        if (fbws != null) {
          byte[] curbts = bts != null ? bts : (byte[]) btarr.get(i);
          hmarr[i] = locked ? fbws[i].tryWriteSafety(curbts)
              : fbws[i].tryWrite(curbts);//throws fttpexception
        } else {
  ...
  public Result<Integer>[] readWriteBatch(TryByteReadAdapter[] ras, boolean readLocked,
      TryByteWriteAdapter[] fwas, boolean writeLocked) {
  ...
  public Result<Integer>[] readWriteIntBatch(TryIntReadAdapter[] ras, boolean readLocked,
      TryIntWriteAdapter[] fwas, boolean writeLocked) {
    return readWriteBatch(null, ras, readLocked, null, fwas, writeLocked);
  }

  private Result<Integer>[] readWriteBatch(TryByteReadAdapter[] tbrs, TryIntReadAdapter[] tirs,
      boolean readLocked, TryByteWriteAdapter[] tbws, TryIntWriteAdapter[] tiws,
      boolean writeLocked) {
    ...
    for (int i = 0, j = 0; j < hmarr.length; ) {
      if (hmarr[i] == null) {
        if (rdarr[i] == null) {
          ...
          if (tbws != null) {
            hmarr[i] = writeLocked ? tbws[i].tryWriteSafety((byte[]) rdarr[i].getResult())
                : tbws[i].tryWrite((byte[]) rdarr[i].getResult());
          } else {
            hmarr[i] = writeLocked ? tiws[i].tryIntWriteSafety((int[]) rdarr[i].getResult())
                : tiws[i].tryIntWrite((int[]) rdarr[i].getResult());
          }
  ...
  public interface TryByteReadAdapter {
    ...
    Result<byte[]> tryReadAll();

    Result<byte[]> tryReadAllSafety();
  }

  public interface TryIntReadAdapter {

    Result<int[]> tryIntReadAll();

    Result<int[]> tryIntReadAllSafety();
  }
  ...
  public interface TryByteWriteAdapter {
    ...
    Result<Integer> tryWrite(byte[] bytes);

    Result<Integer> tryWriteSafety(byte[] bytes);
  }

  public interface TryIntWriteAdapter {

    Result<Integer> tryIntWrite(int[] its);

    Result<Integer> tryIntWriteSafety(int[] its);
  }
}