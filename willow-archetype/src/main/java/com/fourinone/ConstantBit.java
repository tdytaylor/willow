  ...
  enum Target implements ConstantBit, Filter.Condition {
    ...
    private static <T> Target getTarget(Class<T> value, boolean match) {
      try {
        ...
        if (!match && Serializable.class.isAssignableFrom(value)) {
          ...
          LogUtil.info("[CoolHashException]", "[IllegalArgumentException]",
    ...
    static <T> T getTargetObject(byte[] bts, Class<T> value, ByteReadParser brp) {
      if (bts == null || bts.length == 0) {
        ...
        ConstantBit<T> cb =
            value != null ? Target.getTarget(value) : Target.getTarget(bts[bts.length - 1]);
    ...
    public boolean matchAction(Object key, Object value) {
      Filter<String, Object> filter = (Filter) key;
      byte[] vb = (byte[]) value;
      if (vb.length - 1 == codeValue && (vb[vb.length - 1] == codeKey || (
          vb[vb.length - 1] == DumpAdapter.ConstBit[6]
              && codeKey == DumpAdapter.ConstBit[3]))) {//+1? type==type
        Filter.Condition cd = Action.valueOf(filter.getFilterKey());
