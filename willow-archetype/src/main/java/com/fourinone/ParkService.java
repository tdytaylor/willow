...
public class ParkService extends MementoService implements Park {
  ...
  public ParkService(String host, int port, String[][] servers, String parkService)
...
//  }
    ...
    if (domain != null && node != null) {
      ...
      try {
        ...
        if (!parkinfo.containsKey(domainnodekey)) {
          ...
          parkinfo
              .setObj(ParkMeta.getYBB(domain), updateDomainVersion());//updateDomainVersion(domain)
          ...
          LogUtil.info("[Park]", "[create]", domainnodekey + " is exist!");//throw exist exception
        }
      } catch (Exception e) {
        //e.printStackTrace();
      ...
      try {
        ...
        if (checkAuth(domainnodekey, sessionid, AuthPolicy.OP_READ_WRITE)) {
          if (parkinfo.containsKey(domainnodekey)) {
            ...
            if (theversion != parkinfo.getObj(ParkMeta.getYBB(domainnodekey))) {
            ...
            LogUtil.info("[Park]", "[update]",
                domainnodekey + " is not exist!");//throw not exist exception
          }
        }
      } catch (Exception e) {
        //e.printStackTrace();
    ...
    if (domain != null) {
      ...
      try {
        if (parkinfo.containsKey(domain)) {
          ...
          LogUtil.info("[Park]", "[update]", domain + " is not exist!");//throw not exist exception
        }
      } catch (Exception e) {
        LogUtil.info("[Park]", "[update]", e);
      } finally {
  ...
  public ObjValue delete(String domain, String node, String sessionid)
    ...
    if (domain != null) {
      ...
      if (checkAuth(ParkObjValue.getDomainnodekey(domain, node), sessionid, AuthPolicy.OP_ALL)) {
  ...
  public boolean checkAuth(String domainnodekey, String sessionid, AuthPolicy targetauth) {
    ...
    if (creator != null) {
      if (creator.equals(sessionid)) {
        ...
        int thekeyauth =
            domainnodeAuth != null ? (Integer) domainnodeAuth : 1;//only read for domain
  ...
  public ObjValue get(String domain, String node, String sessionid)
    ...
    if (domain != null) {
      ...
      rlk.lock();
      LogUtil.fine("[Park]", "[get]", ParkObjValue.getDomainnodekey(domain, node));
      ov = parkinfo
          .getNode(domain, node);//parkinfo.getNodeWidely(parkinfo.getDomainnodekey(domain, node));
      if (ov.isEmpty()) {
        ov = null;
      }
      //lk.unlock();
  ...
  public ObjValue getLastest(String domain, String node, String sessionid, long version)
    ...
    Lock rlk = rwlk.readLock();
    rlk.lock();
    Long nodeversion = (Long) parkinfo
        .getObj(ParkMeta.getYBB(ParkObjValue.getDomainnodekey(domain, node)));
    if (nodeversion != null && nodeversion != version) {
      LogUtil.fine("[Park]", "[getLastest]", "nodeversion:" + nodeversion + ";version:" + version);
    }
    ObjValue ov =
        (nodeversion != null && nodeversion != version) ? get(domain, node, sessionid) : null;
    //lk.unlock();
    rlk.unlock();
    return ov;
  }
