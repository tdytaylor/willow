...
public class HbDaemo {
  ...
  public static void runPutTask(Park pk, ParkLeader pl, String domain, String node,
      String sessionid) {
    if (putTask == null) {
...
class PutHbTask extends TimerTask {
  ...
  private final ParkLeader pl;
  private final String domain;
  private final String node;
  private final String sessionid;
  private final ArrayList<String> putList = new ArrayList<String>();
  private Park pk;
    ...
    for (String domainnodekey : hbarr) {
      ...
      if (t > HbDaemo.gt) {
        ...
        if (t > HbDaemo.gt + HbDaemo.dt) {
          if (HbDaemo.dt > 0) {
            LogUtil.warn("[Park]", "[Dead]",
                domainnodekey + " has exceeded max delaytime and is regarded as dead by park.");
          }
...
class ClearTask extends TimerTask {
  ...
  public void run() {
    ...
    for (String[] keyarr : expkeys) {
      LogUtil.fine("[Clear]", "[Expiration]", ParkObjValue.getDomainnodekey(keyarr[0], keyarr[1]));
      ps.delete(keyarr[0], keyarr[1]);
    }
  }
}