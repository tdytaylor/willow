...
//2013.8.29
    ...
    if (tasks.length < wks.length) {
      ...
      System.arraycopy(wks, new Random().nextInt(wks.length - tasks.length + 1), wkls, 0,
          tasks.length);
      wks = wkls;
    }
    ...
    for (int i = 0, done = 0; done < tasks_result.length; ) {
      if (wks_result[i] == null) {
        for (int job = 0; job < tasks_result.length; job++) {
          if (tasks_result[job] == null || (!allfault
              && tasks_result[job].getStatus() == WareHouse.EXCEPTION)) {
