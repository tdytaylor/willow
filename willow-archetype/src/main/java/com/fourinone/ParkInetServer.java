  ...
  public void handle(HttpExchange exchange) throws IOException {
    ...
    if (requestMethod.equalsIgnoreCase("GET")) {
      ...
      exchange.sendResponseHeaders(HttpURLConnection.HTTP_OK,
          fl.length());//HttpURLConnection.HTTP_OK,responseMsg.length();
      OutputStream responseBody = exchange.getResponseBody();
    ...
    try {
      ...
      if (treenum == 0) {
        ...
        FttpAdapter fa = new FttpAdapter(
            treeId);//ObjectBytes.getViewUtf8UrlString(treeId)getViewUrlStringdecodeReplace"fttp://localhost"
        //rootlist = fa.listRoots();
        fttpchild = fa.getChildProperty();
    ...
    try {
      ...
      if (auth != null) {
        String authstr = new String(
            ObjectBytes.decode(auth.get(0).split("\u0020")[1].toCharArray()));
      ...
      if (authflag) {
				if (requesturi.equals(ConfigContext.getProp("REQFTTP"))) {
					...
					String tid = ObjectBytes
							.getViewUtf8UrlString(rqtid);//getViewUrlString ObjectBytes.decodeReplace(tid);
