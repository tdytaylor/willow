...
class DumpAdapter extends FileAdapter {
  ...
  final static long ms = mk + m(ConfigContext.getREGIONLENGTH()) + ConstBit[8]
      + ConstBit[5];//mk+mv? or getRegion=max(mv,getConfig)
  ...
  String[] getKeyWildArr(String k) {
    String[] ks = k.split("\\u002E");
    for (int i = 0; i < ks.length; i++) {
      ks[i] = ks[i].indexOf("\u002A") != -1 ? "\u002A" : getMetaString(
          ks[i]); //ks[i].equals("\u002A")?ks[i]:getMetaString(ks[i]);//2015.7.28 indexOf()!=-1;
    }
    ks[ks.length - 1] =
        ks[ks.length - 1].equals("\u002A") ? ks[ks.length - 1] : ks[ks.length - 1] + "\u0030";
    return ks;
  }
