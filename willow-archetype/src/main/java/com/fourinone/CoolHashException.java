...
public class CoolHashException extends ServiceException {
  ...
  public CoolHashException() {
    ...
    pkw = Pattern.compile(
        "^([a-z0-9A-Z_]+|([a-z0-9A-Z_]*\\*{1}[a-z0-9A-Z_]*))(\\.([a-z0-9A-Z_]+|([a-z0-9A-Z_]*\\*{1}[a-z0-9A-Z_]*)))*$");//2015.7.28
  }
  ...
  void checking(String str) throws CoolHashException {
		if (str == null || pk.matcher(str).matches() == false) {
			throw new CoolHashException("[Syntax Error] the keyname '" + str
  ...
  boolean checkWild(String str) {
    if (str == null || pkw.matcher(str).matches() == false) {
      LogUtil.fail("[CoolHashException]", "[Syntax Error]", "the findname '" + str
  ...
  void checkingType(Object key, Object value) throws CoolHashException {
		...
		if (!(key instanceof String && value.getClass().isArray() && value.getClass().getComponentType()
				.equals(byte.class))) {
  ...
  void checkingPlusValue(Object plusValue) throws CoolHashException {
		if (!(plusValue instanceof String) && !(plusValue instanceof Number)) {
			throw new CoolHashException(
					"[TypeErrorException] the plusValue is not instanceof Number or String!");
		}
  }
  ...
  Object getCheckPlus(Object value, Object plusValue) {
		if (plusValue instanceof Short && (value == null || value instanceof Short)) {
			...
			return value == null ? plusValue : value + (String) plusValue;
		} else {
			LogUtil.fail("[CoolHashException]", "[ValuePlus Type Error]",
					"the plusValue is null or not Number&String or is not same type of the value!");
		}
    return null;
  }
  ...
  void checkingCoolBitSet(int index, String logical) throws CoolHashException {
		if (index < 0 || index >= CoolBitSet.defaultMax) {
			throw new CoolHashException(
					"[Exceed Error] the size of the CoolBitSet less-equal 0 or exceed:"
							+ CoolBitSet.defaultMax);
		}

    if (logical != null) {
      String logicallower = logical.toLowerCase();
			if (!logicallower.equals("and") && !logicallower.equals("or") && !logicallower.equals("xor")
					&& !logicallower.equals("andnot") && !logicallower.equals("new")) {
				throw new CoolHashException(
						"[Syntax Error] the logical is not and/or/xor/andnot/new:" + logical);
			}
    }
  }
  ...
  void pointLoopException() {
    LogUtil.fail("[CoolHashException]", "[PointLoopException]",
