...
class DumpCtor extends Contractor implements CoolHashBase, CoolHashClient {
  ...
  private final ConstantBit.Target ct = ConstantBit.Target.POINT;
  private final ByteWriteParser bwp = DumpAdapter.getByteWriteParser();
  private final ByteReadParser brp = DumpAdapter.getByteReadParser();
  private final WareHouse writeAhead = new WareHouse();
  WorkerLocal[] wks;
  ...
  public <T> T get(String key, Class<T> valueType, byte c, boolean point, String... pointSubKey)
      throws CoolHashException {
    chex.checking(key);
    byte[] bts = writeAhead.getMark() && writeAhead.containsKey(key) ? (byte[]) writeAhead.get(key)
        : dump(key, null, c, point, pointSubKey);
  ...
  //2017.10
  private <T> Object put(String key, T value, byte c, boolean point, String... subKey)
      throws CoolHashException {
    byte[] bts =
        value != null ? Target.getTargetBytes(value, bwp) : null;//getTargetBytes(value,bwp,code)
    chex.checking(key, bts);
    bts =
        writeAhead.getMark() ? (byte[]) writeAhead.put(key, bts) : dump(key, bts, c, false, subKey);
    return bts != null ? Target.getTargetObject(bts, brp) : null;
  }
  ...
  public String putPoint(String keyPoint, String key) throws CoolHashException {
    ...
    chex.checking(key);
    byte[] bts = writeAhead.getMark() ? (byte[]) writeAhead.put(keyPoint, ct.getBytes(key, bwp))
        : dump(keyPoint, ct.getBytes(key, bwp), (byte) 0x14, false);
    return bts != null ? Target.getTargetObject(bts, String.class, brp) : null;
  }
  ...
  class CoolResult implements CoolKeyResult, CoolHashResult {
    ...
    CoolResult(String filewild, Filter filter, boolean point, String[] pointSubKey,
        List<File> dfr) {
