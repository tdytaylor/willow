...
public class FttpException extends FileException {
  ...
  static FttpException getNewException(Throwable tw, FileAdapter fa) {
    return (!fa.exists() || !fa.isFile()) ? new FttpException(new FileNotFoundException())
        : new FttpException(tw);
  }
  ...
  static URI getURI(String fttpPath, String filename) throws FttpException {
    ...
    try {
      fl = new URI(fttpPath);
			if (!fl.getScheme().equalsIgnoreCase("fttp")) {
  ...
  static URI getFttpURI(URI ui, String filePath, String filename) {
    ...
    return ui.resolve(filePath != null && filePath.length() != 0 ? (filename == null ? filePath
        : filePath + (filePath.endsWith("\u002F") ? "" : "\u002F") + filename)
        : "");//(new File(filePath,filename)).toURI().getPath()
  }
}